...
const OUTPUT_DIR = process.env.LIVE_METADATA_OUTPUT_DIR
  ? path.resolve(process.env.LIVE_METADATA_OUTPUT_DIR)
  : MONITORING_DIR;
...
const USER_AGENT =
  "CaesarRegulationWatch/0.9.8 (metadata-pilot; +https://regulation-watch.caesar.no/methodology/)";
const FETCH_TIMEOUT_MS = 25_000;
const TITLE_READ_BYTES = 12_288;

const LEGAL_SAFE_NOTE =
  "v0.9.8 cautious live metadata pilot. One request per allowlisted official URL; metadata headers and title only; no full legal text storage. Not scheduled monitoring. Not legal advice. Not client evidence. Human review required for meaningful metadata differences (not weak header null-to-present noise).";
...
async function main() {
  ...
  const runPath = path.join(OUTPUT_DIR, `${RUN_ID}.yml`);
  const packPath = path.join(OUTPUT_DIR, `${PACK_ID}.yml`);
