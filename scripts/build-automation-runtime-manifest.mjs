...
function main() {
  ...
  const manifest = {
    ...
    next_setup_steps: [
      "read docs/runtime/NEXT_DEVELOPMENT_HANDOFF.md for post-activation product work",
      "run npm run runtime:services:check and validate:runtime-services-readiness",
      "run npm run runtime:db:health and validate:runtime-db-health (tracked exports on main)",
      "connect source pilot to runtime DB (T075B) before network execution (T076+)",
      "enable live ingestion, cron, or network execution only after explicit Control Tower approval",
      "do not re-run dev-runtime-activate schema apply or Worker deploy without approval",
    ],
