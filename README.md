...
**Status:** v0.8.6 on **`main`** — **public GitHub Pages pilot** at [caesar-compliance.github.io/caesar-ai-regulation-watch](https://caesar-compliance.github.io/caesar-ai-regulation-watch/) (manual `deploy-static-site.yml` with `DEPLOY` gate; redeploy after data merges); v0.8.6 first source review batch (9 items); v0.8.3 evidence export **candidates** (2 ready for human review, 3 simulation-blocked); monitoring cycle; 5 watchers; CI validate/build (no backend APIs, database, auth, secrets, custom domain, or `client_use_allowed: true`)
