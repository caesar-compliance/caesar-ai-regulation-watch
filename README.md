...
**Status:** v0.9.7 — **public pilot** at [regulation-watch.caesar.no](https://regulation-watch.caesar.no/) (v0.9.7 triages v0.9.6 live metadata review candidates — metadata change is not a legal/regulatory change claim; cautious live metadata pilot for 5 allowlisted official URLs; plus v0.9.5 adapter pack; 3 manual/blocked; not complete coverage; not legal advice; 24 discovery leads confirmed, 1 pending) (manual `deploy-static-site.yml` with `DEPLOY` gate; site root `/`); deployment log in [DEPLOYMENTS.md](DEPLOYMENTS.md); export-candidate governance review gate (2 reviewed, 0 client-use); evidence export **candidates** only (not final evidence); monitoring cycle; CI validate/build (no backend, database, auth, secrets, or `client_use_allowed: true`)
