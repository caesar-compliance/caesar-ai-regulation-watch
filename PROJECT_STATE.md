...
| Field | Value |
|---|---|
| **Current version** | `v1.0.8` (implemented on main — T051 + T052 + T053) |
| **Live version** | `v1.0.7` — tag `regulation-watch-v1.0.7` |
| **Status** | T051, T052, and T053 on main (T053 PR pending merge); live site remains v1.0.7; tag/deploy/closeout pending future Control Tower approval |
...
- **T053 manual source intake runner** — `manual-intake-runs.yml`, validation, fixture-first CLI runner (`edpb-publications-rss` pilot), output under `generated/source-intake-candidates/`. No live network; no scheduling; gates closed.
- **Recommended next** — T054 approved single-source network dry-run design (manual-only, Control Tower approval, no publication).
...
- T052 allowlist + T053 fixture intake only — no live API/RSS ingestion yet.
