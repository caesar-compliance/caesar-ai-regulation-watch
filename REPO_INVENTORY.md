...
This is a living registry of all files currently tracked in the `caesar-ai-regulation-watch` repository. It provides developers and automated agents with a reference mapping each file to its exact role in the codebase structure.
...
| :--- | :--- | :--- |
| 🛡️ **`README.md`** | Main Introduction | Summarizes the project vision, functional mapping workflows, target audience, ecosystem alignment with [caesar-ai-governance-hub](https://github.com/caesar-compliance/caesar-ai-governance-hub), and legal disclaimers. |
| 📋 **`SPEC.md`** | Technical Specification | Defines core mapping scope, inputs, outputs, non-goals, and scheduling boundaries. |
| 🏗️ **`ARCHITECTURE.md`** | System Architecture | Visualizes codebase modular layers, crawler parsers, and controls database mapping structures. |
| 🚦 **`ROADMAP.md`** | Project Roadmap | Lists development phases transitioning the mapping feed from planning stages through stable release. |
...
1. Ensure any new mapping rules or statutory database additions are correctly documented inside `SPEC.md`.
