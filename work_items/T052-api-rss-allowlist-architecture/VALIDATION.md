...
Safety gate script (forbidden flags in allowlist/export/snapshot files) — see FINAL_REPORT.

Optional preview smoke:

```bash
npm run preview -- --host 127.0.0.1 --port 4321
curl -fsSI http://127.0.0.1:4321/source-adapters/
curl -fsSI http://127.0.0.1:4321/data/source-adapter-allowlist.json
```
